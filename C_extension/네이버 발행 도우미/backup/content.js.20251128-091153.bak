...
// capture 단계에서도 핫키를 막아 브라우저 기본 동작을 차단
...
async function runSequence() {
    ...
    // Target datetime (settings-based)
    ...
    // Step 1: publish button (retry for slow load)
    let publishBtn = null;
    for (let i = 0; i < 6 && !publishBtn; i++) {
        publishBtn = findPublishButton(topDoc);
        if (!publishBtn) await sleep(700);
    }
...
function findPublishButton(doc) {
    ...
    // fallback: any element with keyword text
...
function getFollowingSelects(anchor, rootDoc) {
    const doc = rootDoc || window.top.document;
    return Array.from(doc.querySelectorAll(".selectbox_button__jb1Dt, [role='listbox']"))
        .filter((el) => isVisible(el) && (anchor.compareDocumentPosition(el) & Node.DOCUMENT_POSITION_FOLLOWING));
}
...
async function setDateTime(doc, targetDate) {
    ...
    // Fallback: set value directly if text input
...
function findTimeInputs(doc) {
    const hour = Array.from(doc.querySelectorAll("input")).find((el) =>
        isVisible(el) &&
        ((el.getAttribute("aria-label") || "").includes("시") ||
            (el.getAttribute("title") || "").includes("시") ||
            (el.name || "").toLowerCase().includes("hour") ||
            (el.className || "").toLowerCase().includes("hour")) &&
        !el.readOnly
    );
    const minute = Array.from(doc.querySelectorAll("input")).find((el) =>
        isVisible(el) &&
        ((el.getAttribute("aria-label") || "").includes("분") ||
            (el.getAttribute("title") || "").includes("분") ||
            (el.name || "").toLowerCase().includes("min") ||
            (el.className || "").toLowerCase().includes("min")) &&
        !el.readOnly
    );
    return { hour, minute };
}
...
function findTimeSelectButtons(container, anchor) {
    ...
    // fall back to following elements after date anchor
    if (anchor) {
...
async function setTime(doc, dateBtn, targetDate) {
    ...
    let hourSet = false;
    let minSet = false;
    if (hour) { setNativeValue(hour, hourText); hourSet = true; }
    if (minute) { setNativeValue(minute, minText); minSet = true; }

    // 2) Fallback: tab order select boxes right after date field
    if (!hourSet || !minSet) {
        const container = dateBtn.closest(".layer_content_set_publish__KDvaV, .layer_popup__i0QOY, .blog_seone_wrap__C5hlm") || doc;
        const selects = findTimeSelectButtons(container, dateBtn);
        const hourSel = !hourSet ? selects[0] : null;
        const minSel = !minSet ? selects[1] : null;
        if (hourSel) {
            const ok = await pickSelect(hourSel, hourText);
            hourSet = hourSet || ok;
        }
        if (minSel) {
            const ok = await pickSelect(minSel, minText);
            minSet = minSet || ok;
        }
    }

    // 3) Fallback: tab to next fields and set
    if (!hourSet || !minSet) {
        let h = hour;
        let m = minute;
        if (!h) h = getNextTabbable(dateBtn);
