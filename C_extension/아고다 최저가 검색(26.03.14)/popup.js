...
function renderSelectionCounts() {
  trafficCount.textContent = `${collectCheckedIds("selectedTrafficIds").length}개 선택`;
  cardCount.textContent = `${collectCheckedIds("selectedCardIds").length}개 선택`;
  airlineCount.textContent = `${collectCheckedIds("selectedAirlineIds").length}개 선택`;
  promoCount.textContent = `${collectCheckedIds("selectedPromoIds").length}개 선택`;
}
...
async function autoRunIfNeeded() {
  ...
  if (
    ...
    setStatus("이 페이지의 최근 결과가 이미 있습니다.");
    return;
  }
...
async function startScan(openBest, autoStarted = false) {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (!isAgodaPropertyTab(tab?.url)) {
    setStatus("Agoda 호텔 상세 페이지를 먼저 열어 주세요.", true);
    return;
  }
  ...
  setStatus(
    openBest
      ? "결과 탭에서 스캔을 시작했고, 완료되면 1위 링크를 엽니다."
      : "결과 탭에서 스캔을 시작했습니다."
  );
...
async function stopScan() {
  ...
  loadedState = response.scanState || loadedState;
  renderScanState(loadedState);
  setStatus(response.stopped ? "스캔을 중지했습니다." : "진행 중인 스캔이 없습니다.");
}
...
function renderScanState(scanState) {
  ...
  openReservationsButton.disabled = false;

  if (scanState.message) {
    setStatus(scanState.message, /실패|오류|error|failed|agoda/i.test(scanState.message));
  }
...
function buildSummaryText(scanState) {
  ...
  if ((scanState.results || []).length) {
    ...
    return `${base} | 가격 성공 ${successCount}개 | 직접 열기 ${openOnlyCount}개`;
  }
  ...
  return "옵션을 정한 뒤 검색을 시작하세요. 진행 상황과 링크는 결과 탭에서 확인합니다.";
}
