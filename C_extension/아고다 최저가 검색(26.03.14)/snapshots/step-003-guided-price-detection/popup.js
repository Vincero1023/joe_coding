...
async function autoRunIfNeeded() {
  ...
  if (!isAgodaPropertyTab(tab?.url)) {
    setStatus("아고다 숙소 상세 페이지를 열어야 자동 검색이 동작합니다.");
    return;
  }
...
async function startScan(openBest, autoStarted = false) {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (!isAgodaPropertyTab(tab?.url)) {
    setStatus("아고다 숙소 상세 페이지를 먼저 열어주세요.", true);
    return;
  }
  ...
  setStatus(autoStarted ? "자동 검색을 시작했습니다." : openBest ? "검색 후 1위를 엽니다." : "검색을 시작했습니다.");
}
    ...
    return `
      <article class="result-card">
        ...
        <div class="result-actions">
          <button type="button" class="secondary" data-open-url="${escapeHtml(result.scannedUrl)}">이 링크 열기</button>
        </div>
      </article>
    `;
  }).join("");
...
function renderResults(results, currentResult) {
  ...
  results.forEach((result) => {
    ...
    const status = result.ok ? "OK" : result.error || "실패";
    const action = result.ok
      ? `<button type="button" class="mini secondary" data-open-url="${escapeHtml(result.scannedUrl)}">열기</button>`
      : "";
    ...
    row.innerHTML = `
      ...
      <td>
        ...
        <div class="subtle mono">${escapeHtml(result.priceText || "")}</div>
      </td>
      ...
      <td class="${result.ok ? "status-ok" : "status-error"}">${escapeHtml(status)}</td>
      <td>${action}</td>
    `;
...
function buildHintText(results, comparable) {
  ...
  if (!comparable) {
    return "통화가 섞여 있어 순위를 신뢰하기 어렵습니다.";
  }
