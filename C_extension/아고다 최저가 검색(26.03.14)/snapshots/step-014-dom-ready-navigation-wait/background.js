...
const SCAN_PAGE_READY_TIMEOUT_MS = 3200;
const SCAN_POST_READY_DELAY_MS = 900;
  ...
  await new Promise((resolve, reject) => {
    ...
    const refresh = async () => {
      try {
        ...
        if (!latestTab?.id) {
          fail(new Error("The scan tab closed before the Agoda page finished loading."));
          return;
        }
        ...
        if (sawNavigation && latestTab.status === "complete" && latestUrl && latestUrl !== "about:blank") {
          succeed(latestTab);
        }
      } catch (error) {
        fail(error);
    ...
    const timeoutHandle = setTimeout(() => {
      ...
      fail(new Error(`Timed out while loading the Agoda page.${suffix}`));
    }, timeoutMs);
