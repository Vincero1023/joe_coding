...
function injectedScrape(defaultCurrency, waitMs) {
  ...
  const collectAnchorCandidates = (labelPattern, extractor, selectorLabel) => {
    ...
    labelNodes.forEach((node) => {
      buildAnchorTexts(node).forEach(({ text, source }) => {
  ...
  const collectGuidedCandidates = () => {
    ...
    return collectAnchorCandidates(
      /(1\s*\uBC15|1\s*night|per\s*night|nightly|\/night)/i,
  ...
  const readFailureDiagnostics = () => {
    ...
    const startAnchorMatches = visibleText.match(/시작가.{0,60}/g) || [];
    const oneNightMatches = visibleText.match(/.{0,40}1박.{0,40}/g) || [];
    const failureReason = roomTimeoutText
      ? "room-sold-out-or-unavailable"
      : startAnchorMatches.length || oneNightMatches.length
    ...
    if (startAnchorMatches.length) {
      debugParts.push(`start-anchor: ${normalizeText(startAnchorMatches[0])}`);
    }
    if (oneNightMatches.length) {
      debugParts.push(`one-night-anchor: ${normalizeText(oneNightMatches[0])}`);
    }
    ...
    return {
      ...
      priceText: roomTimeoutText || startAnchorMatches[0] || oneNightMatches[0] || ""
    };
  };
  ...
  return (async () => {
    ...
    if (!candidates.length) {
      ...
      return {
        ok: false,
        error: "Could not find a price next to the start-price or one-night anchor.",
        ...failureDiagnostics
      };
    }
