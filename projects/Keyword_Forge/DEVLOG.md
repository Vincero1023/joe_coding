...
## Date
- 2026-03-28 16:20 (KST)

## What changed (변경점)
- 상단 작업 흐름을 `시작모드 / 실행버튼 / 2축선별` 3패널 중심으로 재정리하고, 상세 설정은 `관련 설정 보기` 모달로 분리했다.
- `출력 및 복사`를 플로팅 도크 아래 토글 패널로 옮기고, 단계별 `CSV / TXT / 줄바꿈 복사 / 콤마 복사` 기능을 유지했다.
- `시드화` 버튼으로 확장/분석 결과 키워드를 시드 입력으로 바로 되돌리는 재탐색 루프를 붙였다.
- 수집 설정 안의 Creator Advisor 로그인/세션 블록을 읽기 쉽게 재배치하고, 모달 overflow 문제를 정리했다.
- `/guides`, `/recommended-usage` 문서를 현재 앱 구조 기준으로 다시 작성해 샘플 문구 의존을 끊었다.

## Why (원인/배경)
- 실행에 필요한 핵심 제어가 화면 여기저기에 흩어져 있어, 한 번 돌리기까지 손이 많이 갔다.
- 관련 설정을 본문에 그대로 펼치면 레이아웃이 쉽게 무너졌고, 실제로는 자주 건드리지 않는 옵션이 화면을 복잡하게 만들었다.
- 도움말과 추천 사용법은 최신 UI와 어긋나기 시작해, 현재 앱을 처음 보는 사람이 오히려 헷갈릴 수 있었다.

## How verified (검증 방법/체크리스트)
- [x] 새 `status` HTML과 사용자 캡처 기준으로 레이아웃 겹침, 빈 공간, 모달 overflow, 버튼 위치를 반복 확인
- [x] `pytest -q tests/test_web_routes.py`
- [x] `/guides`, `/guides/dual-axis-selection`, `/recommended-usage` 응답에 `Study`, `키워드마스터`, `샘플` 노출이 남지 않았는지 확인

## Issues & Fix (문제-원인-해결)
- 문제: 관련 설정을 페이지 안에서 펼치면 화면이 길어지고, 오른쪽 패널 높이가 다른 패널 배치를 계속 밀어냈다.
- 원인: 실행 핵심과 세부 설정이 같은 레이아웃 그리드에 함께 놓여 있었다.
- 해결: 핵심 실행 제어는 상단 3패널로 고정하고, 모드별 세부 설정은 전용 모달로 옮겼다.

## Next (다음 작업)
- 브라우저 구독형 LLM을 로컬 브라우저 자동화로 연결하는 `제목 생성 브리지` 설계
- 우선순위는 `Playwright + 전용 브라우저 프로필 + 반자동 회수` 구조로 검토
- CAPTCHA, 로그인 만료, 응답 형식 흔들림에 대비한 수동 개입 포인트 설계
